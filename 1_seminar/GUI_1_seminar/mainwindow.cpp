...
void MainWindow::on_start_all_sort_button_clicked()
{
    ...
    int* bubble_arr = new int[length_arr];
    int* shaker_arr = new int[length_arr];
    int* selection_arr = new int[length_arr];
    ...
    if (flag_parallel) {
        std::thread th1(sort_and_measure, ref(sorter_bubble), bubble_arr, "пузырьком", ref(bubble_time), ref(bubble_comparisons), ref(bubble_swaps));
        std::thread th2(sort_and_measure, ref(sorter_shaker), shaker_arr, "шейкером", ref(shaker_time), ref(shaker_comparisons), ref(shaker_swaps));
        std::thread th3(sort_and_measure, ref(sorter_selection), selection_arr, "выбором", ref(selection_time), ref(selection_comparisons), ref(selection_swaps));
    ...
    double sortData[9] = {bubble_time, bubble_comparisons, bubble_swaps, shaker_time, shaker_comparisons, shaker_swaps, selection_time, selection_comparisons, selection_swaps};
